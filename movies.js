...
window.addEventListener("DOMContentLoaded", async function (event) {
  ...
  let response = await fetch(
    "https://api.themoviedb.org/3/movie/now_playing?api_key=f1051e72017bf09471356a5604a3affd&language=en-US"
  );
  let json = await response.json();
  let movies = [json];
  console.log(movies);
  let moviesdata = movies[0].results;
  ...
  for (let i = 0; i < moviesdata.length; i++) {
    let movieID = moviesdata[i].id;
    let poster = moviesdata[i].poster_path;
    //console.log(movieID , poster)
    document.querySelector(".movies").insertAdjacentHTML(
      ...
      `<div class="movies-${movieID} w-1/5 p-4 ">
         ...
         <a href="#${movieID}" class="watched-button block text-center text-white bg-green-500 mt-4 px-4 py-2 rounded">Watched</a>
       </div>`
    );
    let docOpac = db.collection("Movies").doc(`${movieID}`);

    docOpac
      .get()
      .then((doc) => {
        if (doc.data().Opacity == "opacity-20") {
          console.log("Opacity:", doc.data().Opacity);
          document
            .querySelector(`.movies-${movieID}`)
  ...
  buttons.forEach(function (currentBtn) {
    currentBtn.addEventListener("click", async function (event) {
      ...
      console.log(HREF);
      document.querySelector(`.movies-${HREF}`).classList.add("opacity-20");
      console.log(`Movie ${HREF} was watched.`);

      var docRef = db.collection("Movies").doc(`${HREF}`);
